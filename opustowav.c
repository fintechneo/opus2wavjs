...
int opustowav(const char *infile,const char *outfile){
  OggOpusFile  *of;
  ...
  output_seekable=fseek(outf,0,SEEK_CUR)!=-1;
  if(op_seekable(of)){
    opus_int64  size;
  ...
  if(!fwrite(wav_header,sizeof(wav_header),1,outf)){
    ...
    for(;;){
      ...
      if(ret<0){
        fprintf(stderr,"\nError decoding '%s': %i, buffersize: %i\n",infile,ret,sizeof(pcm)/sizeof(*pcm));
      ...
      if(!fwrite(out,sizeof(*out)*4*ret,1,outf)){
    ...
    if(op_seekable(of)&&nsamples!=duration){
      fprintf(stderr,"\nWARNING: "
       "Number of output samples %li does not match declared file duration %li.\n",(long)nsamples,(long)duration);
    ...
    if(output_seekable&&nsamples!=duration){
      ...
      if(fseek(outf,0,SEEK_SET)||
       !fwrite(wav_header,sizeof(wav_header),1,outf)){
